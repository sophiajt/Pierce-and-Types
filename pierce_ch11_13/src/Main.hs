...
data Value = Lambda (Variable, Type, Term) | NilValue Type | ConsValue Type Value Value | TrueValue | FalseValue | ErrorValue | ValueTerm Term deriving Show
data Type = TFun Type Type | TList Type | TBool | TUnknown deriving (Show, Read, Eq)
data Term = Var Variable | Abs Variable Type Term | App Term Term | Nil Type | Cons Type Term Term | IsNil Type Term | Head Type Term | Tail Type Term | TrueT | FalseT | ErrorT deriving (Show, Read, Eq)
...
eval (Cons type1 t1 t2) = 
  let t1_eval = eval t1 in
    let t2_eval = eval t2 in 
      if (t1_eval == t1) && (t2_eval == t2) 
        then (Cons type1 t1_eval t2_eval)
        else eval (Cons type1 t1_eval t2_eval)
eval (IsNil type1 t1) = 
  let t1_eval = eval t1 in
    if (t1_eval == t1) 
      then case t1_eval of (Nil _)      -> TrueT
                           (Cons _ _ _) -> FalseT
                           _            -> ErrorT
      else eval (IsNil type1 t1_eval)
eval (Head type1 t1) = 
  let t1_eval = eval t1 in
    if (t1_eval == t1) 
      then case t1_eval of (Cons cons_type cons_v1 cons_v2) -> eval cons_v1
                           _                                -> ErrorT
      else eval (Head type1 t1_eval)
eval (Tail type1 t1) = 
  let t1_eval = eval t1 in
    if (t1_eval == t1) 
      then case t1_eval of (Cons cons_type cons_v1 cons_v2) -> eval cons_v2
                           _                                -> ErrorT
      else eval (Tail type1 t1_eval)
...
eval_to_value t =
  let evaled_t = eval t in
    case evaled_t of (Abs var type1 term) -> Lambda (var, type1, term)
                     ...
                     FalseT -> FalseValue
                     Var _ -> ErrorValue
                     Nil type1 -> NilValue type1
                     Cons type1 t1 t2 -> ConsValue type1 (eval_to_value t1) (eval_to_value t2)
                     term -> ValueTerm term

typeCheck::Term->Context->Type
typeCheck TrueT _  = TBool
typeCheck FalseT _ = TBool
typeCheck (Nil type1) _ = TList type1
typeCheck (Cons type1 t1 t2) ctx = 
  let t1_type = typeCheck t1 ctx in
    let t2_type = typeCheck t2 ctx in
      if (t1_type /= type1) || (t2_type /= (TList type1)) then TUnknown else TList type1
typeCheck (IsNil type1 t1) ctx = 
  let t1_type = typeCheck t1 ctx in
    if (t1_type /= (TList type1)) then TUnknown else TBool
typeCheck (Head type1 t1) ctx = 
  let t1_type = typeCheck t1 ctx in
    if (t1_type /= (TList type1)) then TUnknown else type1
typeCheck (Tail type1 t1) ctx = 
  let t1_type = typeCheck t1 ctx in
    if (t1_type /= (TList type1)) then TUnknown else (TList type1)
