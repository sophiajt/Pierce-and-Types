...
data Value = Lambda (Variable, Type, Term) | InlValue Value | InrValue Value | TrueValue | FalseValue | ErrorValue | ValueTerm Term deriving Show
data Type = TFun Type Type | TSum Type Type | TBool | TUnknown deriving (Show, Read, Eq)
data Term = Var Variable | Abs Variable Type Term | App Term Term | Inl Term | Inr Term | CaseInl Term Variable Term | CaseInr Term Variable Term | TrueT | FalseT | ErrorT deriving (Show, Read, Eq)
...
eval (Inl t1) = Inl (eval t1)
eval (Inr t1) = Inr (eval t1)
eval (CaseInl t1 varName t2) = 
  let t1_eval = eval t1 in
    if t1_eval == t1
      then case t1_eval of (Inl inl_eval) -> substitute varName inl_eval t2
                           _              -> ErrorT
      else eval (CaseInl (eval t1) varName t2)
eval (CaseInr t1 varName t2) = 
  let t1_eval = eval t1 in
    if t1_eval == t1
      then case t1_eval of (Inr inr_eval) -> substitute varName inr_eval t2
                           _              -> ErrorT
...
typeCheck (Inl t1) ctx = TSum (typeCheck t1 ctx) TUnknown 
typeCheck (Inr t1) ctx = TSum TUnknown (typeCheck t1 ctx)
