...
data Term = Var Variable | Abs Variable Type Term | App Term Term | Try Term Term | TrueT | FalseT | ErrorT deriving (Show, Read, Eq)
...
eval::Term->Term
eval (App (Abs var type1 t1) ErrorT) = ErrorT 
...
eval (App ErrorT t2) = ErrorT
eval (App t1 t2) = eval (App (eval t1) t2)
eval (Try t1 t2) = 
  let t1_eval = eval t1 in
    if (t1_eval == ErrorT) then eval t2 else t1_eval
eval t = t
